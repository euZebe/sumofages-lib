...
describe('Calculator from ages function', () => {
  ...
  it('should return X years later when today is the only participant\'s birthday and expected is +X', () => {
    const moment36YearsAgo = moment().add(-36, 'years').startOf('day');
    const moment36YearOldParticipant = new Participant(moment36YearsAgo);
    const EXPECTED_AGE = 40;
    const nextBirthday = moment(moment36YearOldParticipant.dateOfBirth).add(EXPECTED_AGE, 'years');
    expect(getDateForAccruedAges(EXPECTED_AGE, moment36YearOldParticipant)).toEqual(nextBirthday);
  });

  it('should return the date of the next birthday when expecting the age after', () => {
    const moment3YearsAnd8MonthsAgo = moment().year(now.year() - 3).month(now.month() - 8);
    const moment3YearOldParticipant = new Participant(moment3YearsAnd8MonthsAgo);
    const twoBirthdaysLater = moment(moment3YearOldParticipant.dateOfBirth).add(5, 'years');
    expect(getDateForAccruedAges(5, moment3YearOldParticipant)).toEqual(twoBirthdaysLater);
  });

  it('should return the date of the previous birthday when expecting the current age', () => {
    const moment3YearsAnd8MonthsAgo = moment().year(now.year() - 3).month(now.month() - 10);
    const moment3YearOldParticipant = new Participant(moment3YearsAnd8MonthsAgo);
    const previousBirthday = moment(moment3YearOldParticipant.dateOfBirth).add(2, 'years');
    expect(getDateForAccruedAges(2, moment3YearOldParticipant).startOf('day')).toEqual(previousBirthday);
  });
  ...
  it('should return next birthday when two participants and expected is sum + 1', () => {
    ...
    const nextBirthdayOf3YearOldParticipant = moment(moment3YearOldParticipant.dateOfBirth).add(4, 'years');

    expect(getDateForAccruedAges(40, moment36YearOldParticipant, moment3YearOldParticipant))
